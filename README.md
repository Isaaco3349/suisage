SuiSage
Intelligence on Sui — Autonomous DeFi Agent on DeepBook
SuiSage is an AI-powered trading agent that lives onchain on Sui. It holds its own wallet, executes limit orders on DeepBook via natural language commands, and logs every trade permanently to Walrus decentralized storage.
> Built for Sui Overflow 2026 — submitted under **Agentic Web** and **DeFi & Financial Rails** tracks.
---
What it does
Natural language trading — tell SuiSage "buy 10 SUI at market" or "set a stop loss at 5% below entry" and it figures out the rest
Autonomous agent loop — the agent fetches the orderbook, reasons about the market, and places/cancels orders on DeepBook v3
...
Agent loop (src/lib/agent.ts)
    │  Groq LLM (llama-3.3-70b) + tool use
...
# Fill in GROQ_API_KEY and SUI_PRIVATE_KEY
```
Generate a new Sui keypair for the agent:
```bash
sui keytool generate ed25519
# Fund the address at https://faucet.sui.io
```
