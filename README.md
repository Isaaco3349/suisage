...
Onchain trade log	Every trade signal stored as a blob on Walrus with a verifiable link
...
Agent loop (src/lib/agent.ts)
    ...
    │  Market data injected as context (no tool call errors)
...
---
Tracks
Track	Why SuiSage qualifies
Agentic Web	Autonomous agent with its own Sui wallet — reads live market data, reasons using LLM, emits onchain trade signals, acts without manual input
DeFi & Financial Rails	Native DeepBook v3 integration — live orderbook, spread analysis, multi-pool comparison (SUI/USDC, DEEP/SUI), position management
Walrus (sponsored bounty)	Every trade signal the agent emits is stored as an immutable blob on Walrus testnet — retrievable via aggregator URL, shown live in the UI
---
Move Contract
...
```
> **Note:** Contract is written for testnet-v1.73.1 and ready to deploy. Testnet deployment pending resolution of network-level GitHub git clone restriction in build environment.
---
Stack
Layer	Technology
...
Sui wallet with testnet SUI — faucet.sui.io
1. Clone and install
```bash
...
```
4. Deploy Move contracts (requires Sui CLI)
```bash
cd ../move
sui move build
...
---
Team
Built by @Isaaco3349 for Sui Overflow 2026.
---
License
MIT